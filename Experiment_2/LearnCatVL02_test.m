...
if mod(sub,4) == 0
    tmplist1 = Shuffle(dir('TaskImages/Female*.jpg'));
    tmplist2 = Shuffle(dir('TaskImages/Int*.jpg'));
elseif mod(sub,4) == 1
    tmplist1 = Shuffle(dir('TaskImages/Female*.jpg'));
    tmplist2 = Shuffle(dir('TaskImages/Ext*.jpg'));
elseif mod(sub, 4) == 2
    tmplist1 = Shuffle(dir('TaskImages/Male*.jpg'));
    tmplist2 = Shuffle(dir('TaskImages/Int*.jpg'));
elseif mod(sub, 4) == 3
    tmplist1 = Shuffle(dir('TaskImages/Male*.jpg'));
    tmplist2 = Shuffle(dir('TaskImages/Ext*.jpg'));
...
tmp1 = floor(tmp1/2);
tmp2 = length(tmplist2);
tmp2 = floor(tmp2/2);

imagefiles = cell(1,4); 
imagefiles{1} = tmplist1(1:tmp1);
imagefiles{2} = tmplist1((tmp1+1):end);
imagefiles{3} = tmplist2(1:tmp2);
imagefiles{4} = tmplist2((tmp2+1):end);

% imagefiles = cell(1,4); 
% imagefiles{1} = Shuffle(dir('TaskImages/Female*.jpg'));
% imagefiles{2} = Shuffle(dir('TaskImages/Male*.jpg'));
% imagefiles{3} = Shuffle(dir('TaskImages/Int*.jpg'));
% imagefiles{4} = Shuffle(dir('TaskImages/Ext*.jpg')); 

% open/set up screen
[w,wrect] = Screen('OpenWindow', max(Screen('Screens')),[128 128 128]);
...
for block = 1:nBlocks
    ...
    if block == 1
        DrawFormattedText(w,'If face: Press z for female, m for male.\nIf scene: Press z for interior, m for exterior.\nSpace to start.\nPress q and p simultaneously at any time to quit.','center','center',[0 0 0]);
    else
...
recInst = ['You are now entering the last stage of the experiment.\n' ...
           ...
           'That is, 16 particular face or scene images were ALWAYS\nfollowed by the same face or scene.\n' ...
           ...
           'Half the pairs are new, but will repeat during this stage.\nAlways pick the sequence that appeared during the task you just finished.\n' ...
